...
// Get sale data from session or URL parameter
$sale_data = $_SESSION['last_sale'] ?? null;
$receipt_number_from_url = $_GET['receipt'] ?? $_GET['receipt_number'] ?? '';

// If receipt number provided in URL, fetch from database
if ($receipt_number_from_url) {
    $receipt_number = $conn->real_escape_string($receipt_number_from_url);
    
    // Get first payment record for this receipt to get sale info
    $sale_info_sql = "SELECT * FROM payments WHERE receipt_number = '$receipt_number' LIMIT 1";
    ...
    if ($sale_info_result && $sale_info_result->num_rows > 0) {
        ...
        $sale_data = [
            ...
            'grade' => '', // Will be filled from book data
            'payment_method' => $sale_info['payment_method'],
            'notes' => $sale_info['notes'],
            'total_amount' => 0, // Will be calculated
            'books_sold' => 0, // Will be calculated
...
// Get the most recent sale details from database using receipt number
...
$recent_sales_sql = "SELECT p.*, b.title, b.author, b.price 
                     FROM payments p 
...
<body>
  ...
  <div class="actions">
    ...
    <a href="sell_books.php" class="btn btn-back">🛒 New Sale</a>
